...
- **AFFINE Integration**: Instead of forking the massive AFFINE codebase (which is complex to strip down for a SaaS), we will build a **lightweight, purpose-built workspace** using modern block editors. This ensures you own the stack and can iterate fast on the _SaaS_ features (automation/AI) rather than maintaining a document editor fork.
