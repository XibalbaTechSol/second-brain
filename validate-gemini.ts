...
async function validateGemini() {
  ...
  const testCases = [
    ...
    {
      content:
        "John Smith, Lead Developer at Innovation Corp, john.smith@example.com",
      expectedType: "PERSON",
  ...
  for (const testCase of testCases) {
    ...
    while (attempts < maxAttempts) {
      const check = await prisma.inboxItem.findUnique({
        where: { id: item.id },
      });
      ...
      if (check?.status === "COMPLETED" && check.processedEntityId) {
        ...
        if (entity) {
          ...
          console.log(
            `   ✅ Type: ${entity.type} (Expected: ${testCase.expectedType})`,
          );
          ...
          if (entity.type !== testCase.expectedType) {
            console.warn(
              `   ⚠️ Type mismatch: Got ${entity.type}, expected ${testCase.expectedType}`,
            );
          }
        }
        processed = true;
