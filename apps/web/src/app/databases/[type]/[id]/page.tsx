      ...
      <div className="max-w-4xl mx-auto p-12 py-24 space-y-12">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-[140px_1fr] gap-y-4 text-sm border-t border-b border-border py-8">
            <div className="text-muted-foreground font-bold uppercase text-[10px] tracking-widest">
              Created
            </div>
            ...
            <div className="flex gap-2">
              {entity.tags.map((t) => (
                <span
                  key={t.id}
                  className="bg-primary/10 text-primary px-2 py-0.5 rounded text-xs border border-primary/20"
                >
                  {t.name}
                </span>
              ))}
              {entity.tags.length === 0 && (
                <span className="text-muted-foreground/40 italic">No tags</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="prose dark:prose-invert max-w-none">
          <Editor
            initialContent={
              entity.content ||
              `<h3>Summary</h3><p>${entity.summary || "Start writing..."}</p>`
            }
          />
        </div>
      </div>
