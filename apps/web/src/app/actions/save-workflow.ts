...
export async function saveWorkflow(nodes: any[], edges: any[]) {
  ...
  console.log(
    `Saving workflow for user ${userId}...`,
    nodes.length,
    edges.length,
  );
  ...
  const triggerNodes = nodes.filter(
    (n) =>
      n.type === "aiRouter" || n.type === "schedule" || n.type === "trigger",
  );
  ...
  for (const triggerNode of triggerNodes) {
    ...
    for (const edge of outgoingEdges) {
        ...
        if (node.type === "reasoning") {
          ...
          actions.push({
            type:
              ...
              "create_" + (edge.sourceHandle || "task").toLowerCase(),
            params: {
      ...
      if (actions.length > 0) {
        ...
        console.log(
          `Created workflow for user ${userId} route ${triggerType} with ${actions.length} actions`,
        );
      }
    }
  }
