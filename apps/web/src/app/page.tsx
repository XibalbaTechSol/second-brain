    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/30 transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border transition-colors">
        <div className="max-w-7xl mx-auto px-6 h-24 flex justify-between items-center">
          <div className="flex items-center gap-3 group cursor-pointer">
            ...
            <span className="font-black text-3xl tracking-tighter italic text-foreground transition-colors">
              CognitoFlow
            </span>
          </div>
          ...
          <div className="hidden md:flex gap-10 items-center">
            ...
            {user ? (
              <Link
                href="/dashboard"
                className="bg-primary text-primary-foreground px-10 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs hover:opacity-90 transition-all shadow-xl shadow-primary/20 active:scale-95"
              >
                Dashboard
              </Link>
            ) : (
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-muted-foreground text-xl md:text-2xl max-w-2xl mx-auto mb-12 leading-relaxed transition-colors">
              Most tools just archive information. CognitoFlow uses Gemini 2.0
              to
              <span className="text-foreground font-bold italic">
                {" "}
                analyze, connect, and nudge
              </span>{" "}
              you toward your goals automatically.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link
                href={user ? "/dashboard" : "/login"}
                className="w-full sm:w-auto bg-primary text-primary-foreground px-10 py-5 rounded-2xl text-xl font-black hover:opacity-90 transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-3 group"
              >
                {user ? "Enter Brain" : "Start 7-Day Free Trial"}{" "}
      ...
      <section className="py-12 border-y border-border bg-card/30">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-8">
            Trusted by knowledge workers at
          </p>
      ...
      <section
        id="features"
        className="py-32 px-6 max-w-7xl mx-auto transition-colors"
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div
              key={i}
              className="bg-card border border-border p-10 rounded-[3rem] hover:border-primary/50 transition-all duration-500 group shadow-sm hover:shadow-xl hover:shadow-primary/5"
            >
              <div
                className={`p-4 rounded-2xl w-fit mb-8 group-hover:scale-110 transition-transform duration-500 ${feature.color.replace("aurora-", "")}`}
              >
      ...
      <section
        id="pricing"
        className="py-32 px-6 bg-card/50 border-y border-border transition-colors"
      >
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-black mb-6 italic tracking-tight text-foreground transition-colors">
              Pricing for the AI Era
            </h2>
            <p className="text-muted-foreground text-xl max-w-2xl mx-auto">
              Choose the intelligence level that matches your ambition.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            ...
            <div className="bg-background border border-border p-10 rounded-[3.5rem] flex flex-col transition-colors shadow-sm">
              <h3 className="text-sm font-black text-muted-foreground mb-2 uppercase tracking-[0.2em]">
                Basic
              </h3>
              <div className="text-5xl font-black mb-8 text-foreground">
                $19
              ...
              <Link
                href="/login"
                className="w-full bg-muted text-foreground font-bold py-4 rounded-2xl hover:bg-border transition-all text-center"
              >
                Get Started
              </Link>
            </div>
            ...
            <div className="bg-primary p-10 rounded-[3.5rem] flex flex-col transform md:-translate-y-6 shadow-2xl shadow-primary/30 relative overflow-hidden text-primary-foreground border-4 border-accent/20">
              <div className="absolute top-6 right-10 bg-accent text-accent-foreground text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-tighter shadow-lg">
                Recommended
              </div>
              <h3 className="text-sm font-black opacity-80 mb-2 uppercase tracking-[0.2em]">
                Pro
              </h3>
              <div className="text-5xl font-black mb-8">
                $49<span className="text-lg opacity-60 font-medium">/mo</span>
              </div>
              <ul className="space-y-5 mb-12 flex-1">
                {[
              ...
              <Link
                href="/login"
                className="w-full bg-primary-foreground text-primary font-black py-5 rounded-2xl hover:scale-[1.02] transition-all text-center shadow-2xl"
              >
                Start 7-Day Trial
              </Link>
            </div>
      ...
      >
        <div className="bg-card border border-border rounded-[3.5rem] p-12 md:p-20 relative overflow-hidden shadow-2xl shadow-primary/5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
            <div>
              <h2 className="text-4xl md:text-5xl font-black mb-8 italic tracking-tight text-foreground transition-colors">
                Built for <br />
                Infrastructure
              </h2>
              <p className="text-muted-foreground text-lg mb-10 leading-relaxed transition-colors font-medium">
                CognitoFlow isn't just a web app. It's an intelligence layer.
                Connect your entire ecosystem via our programmatic API.
              </p>
                ...
                {[
                  {
                    ...
                    desc: "Inject thoughts and data from CLI tools or scripts.",
                  },
                  {
                    title: "Secure Keys",
                    desc: "User-specific infrastructure keys with easy rotation.",
                  },
                  {
                    title: "Webhook Ready",
                    desc: "Integrate with any service that supports HTTP POST.",
                  },
                ].map((item, i) => (
                  <div key={i} className="flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground transition-colors font-medium">
                        {item.desc}
                      </p>
                    </div>
                  </div>
    ...
    "source": "Github-Action"
      ...
      <footer className="py-24 border-t border-border text-center bg-background transition-colors">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-muted-foreground text-xs font-medium tracking-[0.1em] opacity-50 transition-colors italic">
