          ...
          {displayedEntities.map((ent) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-sm font-medium text-foreground truncate w-40">
                  {ent.title}
                </span>
              </div>
              {ent.score && (
                <span className="text-xs text-aurora-green font-mono">
                  {(ent.score * 100).toFixed(0)}%
                </span>
              )}
              <ChevronRight
                className={`w-4 h-4 text-muted-foreground group-hover:text-foreground ${selectedEntityId === ent.id ? "text-primary" : ""}`}
              />
            </button>
          ))}
        </div>
          ...
          <div className="flex-1 bg-background relative">
            ...
            <div className="absolute top-6 right-6 flex gap-2">
              ...
              >
                ...
                <span className="text-sm font-medium">
                  {isLinkMode ? "Click target node..." : "Link Nodes"}
                </span>
              </button>
            </div>
            ...
            <div className="absolute bottom-6 left-6 bg-card/80 backdrop-blur p-4 rounded-xl border border-border text-[10px] font-black uppercase tracking-widest space-y-2">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-primary" /> Projects
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-aurora-red" /> People
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-aurora-yellow" /> Ideas
              </div>
              <div className="flex items-center gap-2">
            ...
            {selectedEntity ? (
                ...
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {saveStatus === "saving" && (
                        <span className="text-[10px] font-bold text-muted-foreground flex items-center gap-1 uppercase tracking-widest">
                          <SaveIcon className="w-3 h-3 animate-spin" />{" "}
                          Saving...
                        </span>
                      )}
                      {saveStatus === "saved" && (
                        <span className="text-[10px] font-bold text-aurora-green flex items-center gap-1 uppercase tracking-widest">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="p-2 text-muted-foreground hover:text-aurora-red rounded-lg hover:bg-aurora-red/10 transition-colors"
                    >
              ...
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest">
                  Select a node in the graph or list
                </p>
              </div>
            )}
          </div>
