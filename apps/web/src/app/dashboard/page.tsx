...
export default function ReimaginedDashboard() {
  ...
  const aiCoachItems = inboxItems.filter(
    (i) => i.source === "AI_COACH" || i.source === "AI_REASONING",
  );
  ...
  const avgProgress =
    projects.length > 0
      ? Math.round(
          projects.reduce((acc, p) => acc + (p.project?.progress || 0), 0) /
            projects.length,
        )
      : 0;
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-border pb-8">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-foreground italic flex items-center gap-3 transition-colors">
            Intelligence{" "}
            <span className="text-primary not-italic tracking-normal font-medium">
              Snapshot
            </span>
          </h1>
        ...
        <div className="flex gap-4">
          <div className="bg-card border border-border p-4 rounded-3xl shadow-sm flex items-center gap-4 transition-colors">
            ...
            <div>
              ...
              <div className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest transition-colors">
                Active Projects
              </div>
            </div>
          </div>
          ...
          <section className="bg-primary p-8 rounded-[2.5rem] shadow-xl shadow-primary/20 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h2 className="text-2xl font-black mb-6 flex items-center gap-2 italic tracking-tight">
                <Sparkles className="w-6 h-6 text-accent" /> Frictionless
                Capture
              </h2>
              ...
              <div className="mt-4 flex gap-4 text-[10px] font-bold uppercase tracking-widest text-white/60">
                <span className="flex items-center gap-1.5">
                  <Zap className="w-3 h-3" /> Auto-Routing active
                </span>
                <span className="flex items-center gap-1.5">
                  <Clock className="w-3 h-3" /> Processing:{" "}
                  {pendingInbox.length} items
                </span>
              </div>
            </div>
          </section>
            ...
            <div className="space-y-3">
              {aiCoachItems.length === 0 ? (
                <div className="bg-card/50 border-2 border-dashed border-border rounded-[2rem] p-12 text-center text-muted-foreground transition-colors">
                  Your AI Coach is currently observing. Capture more data to
                  trigger insights.
                </div>
              ) : (
                aiCoachItems.slice(0, 5).map((item) => (
                  <div
                    key={item.id}
                    className="bg-card border border-border p-6 rounded-[2rem] flex gap-4 items-start hover:border-primary/30 transition-all group shadow-sm transition-colors"
                  >
                    <div
                      className={`p-3 rounded-2xl shrink-0 ${item.source === "AI_COACH" ? "bg-aurora-purple/10 text-aurora-purple" : "bg-primary/10 text-primary"}`}
                    >
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <section className="bg-card border border-border rounded-[2.5rem] p-8 shadow-sm transition-colors">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black italic tracking-tight text-foreground transition-colors">
                Active Goals
              </h3>
              <Link
            ...
            <div className="space-y-6">
              {projects.length === 0 ? (
                <div className="text-center py-10 text-muted-foreground text-sm font-medium transition-colors">
                ...
                projects.slice(0, 4).map((proj) => (
                  <div key={proj.id} className="space-y-3">
                    <div className="flex justify-between items-end">
                      <span className="font-bold text-sm text-foreground truncate max-w-[180px] transition-colors">
                        {proj.title}
                      </span>
                      <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest transition-colors">
                        {proj.project?.progress || 0}%
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] font-bold text-primary transition-colors">
                      ...
                      {proj.project?.nextAction || "Define next step..."}
                    </div>
                  </div>
                ))
              )}
              ...
              <div className="bg-background/50 border border-border rounded-2xl p-4 space-y-3">
                ...
                <div className="space-y-3">
                  {[
                    {
                      time: "10:00 AM",
                      label: "Project Momentum Check",
                      type: "Nudge",
                    },
                    {
                      time: "02:30 PM",
                      label: "Idea Scaling Insight",
                      type: "Logic",
                    },
                    {
                      time: "Tomorrow",
                      label: "Admin Efficiency Audit",
                      type: "Automation",
                    },
                  ].map((event, i) => (
                    <div key={i} className="flex gap-3 items-center group">
                      <div className="text-[9px] font-mono text-muted-foreground w-14 shrink-0">
                        {event.time}
                      </div>
                      <div className="flex-1 text-[11px] font-bold text-foreground/80 truncate group-hover:text-primary transition-colors">
                        {event.label}
                      </div>
                      <div className="px-1.5 py-0.5 rounded-md bg-muted text-[8px] font-black uppercase text-muted-foreground">
                        {event.type}
                      </div>
                    </div>
                  ))}
          ...
          <section className="bg-card border border-border rounded-[2.5rem] p-8 shadow-sm transition-colors">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground mb-6 transition-colors font-medium transition-colors">
            ...
            <div className="relative w-40 h-40 mx-auto flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[8px] font-black uppercase text-muted-foreground tracking-tighter transition-colors">
                  Entities
                </span>
              </div>
            </div>
            ...
            <div className="mt-8 space-y-3">
              {[
                ...
                },
                {
                  label: "People",
                  value: entities.filter(
                    (e) => e.type === "PERSON" || e.type === "CONTACT",
                  ).length,
                ...
                >
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${stat.color.replace("aurora-", "")}`}
                    />
                    <span className="text-xs font-bold text-muted-foreground transition-colors">
                      {stat.label}
                    </span>
                  </div>
                  <span className="text-xs font-black text-foreground transition-colors">
                    {stat.value}
                  </span>
                </div>
              ))}
