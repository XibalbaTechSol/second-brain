...
const NodeWrapper = memo(
  ({ children, title, icon: Icon, colorClass, isProcessing, data }: any) => {
    const [isExpanded, setIsExpanded] = React.useState(false);
    const events = data?.events || [];

    return (
      <div
        className={`min-w-[220px] rounded-2xl border-2 bg-card text-card-foreground shadow-xl transition-all relative overflow-hidden
      ...
      >
        ...
        <div className="flex items-center gap-2.5 p-3 border-b border-border bg-muted/30 rounded-t-2xl relative">
          <div
            className={`p-2 rounded-xl bg-background border border-border text-foreground shadow-sm`}
          >
            <Icon
              className={`w-4 h-4 ${isProcessing ? "animate-pulse text-primary" : ""}`}
            />
          </div>
            ...
            {isExpanded && (
              <div className="p-3 space-y-3 max-h-[200px] overflow-y-auto">
                {events.map((event: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex flex-col gap-1 border-l-2 border-primary/30 pl-3 py-1 bg-background/40 rounded-r-lg"
                  >
                    ...
                    <span className="text-[10px] text-foreground/80 leading-tight font-medium">
                      {event.detail}
                    </span>
                  </div>
                ))}
  ...
  return (
    ...
    >
      ...
      <Handle
        type="source"
        position={Position.Right}
        className="!bg-aurora-orange w-3 h-3 border-2 border-background"
      />
    </NodeWrapper>
  );
});
...
export const AIRouterNode = memo(({ data = {} }: any) => {
  return (
    <NodeWrapper
      title={data?.label || "AI Router"}
      icon={GitBranch}
      colorClass="border-primary/30 shadow-primary/5"
      isProcessing={data?.isProcessing}
      data={data}
    >
      ...
      <div className="mt-6 flex flex-col gap-3">
        {["PROJECT", "PERSON", "IDEA", "ADMIN", "GOAL"].map((type) => (
          ...
          >
            <span className="text-[9px] font-black mr-3 text-muted-foreground lowercase tracking-widest group-hover:text-primary transition-colors">
              {type}
            </span>
            <Handle
              type="source"
              position={Position.Right}
              id={type}
              className="w-2.5 h-2.5 !right-[-12px] bg-muted border-2 border-background hover:bg-primary transition-colors"
            />
          </div>
        ))}
      </div>
...
export const ConditionNode = memo(({ data = {} }: any) => {
  return (
    <NodeWrapper
      title={data?.label || "Logical Split"}
      icon={Split}
      colorClass="border-border"
      isProcessing={data?.isProcessing}
      data={data}
    >
      ...
      <div className="space-y-4">
        <div className="relative flex justify-end items-center group">
          <span className="text-[10px] font-black mr-3 text-aurora-green uppercase tracking-widest opacity-70 group-hover:opacity-100">
            True
          </span>
          <Handle
            type="source"
            position={Position.Right}
            id="true"
            className="!bg-aurora-green w-2.5 h-2.5 !right-[-12px] border-2 border-background"
          />
        </div>
        <div className="relative flex justify-end items-center group">
          <span className="text-[10px] font-black mr-3 text-aurora-red uppercase tracking-widest opacity-70 group-hover:opacity-100">
            False
          </span>
          <Handle
            type="source"
            position={Position.Right}
            id="false"
            className="!bg-aurora-red w-2.5 h-2.5 !right-[-12px] border-2 border-background"
          />
        </div>
      </div>
    </NodeWrapper>
  ...
  return (
    ...
    >
      ...
      <Handle
        type="target"
        position={Position.Left}
        className="!bg-aurora-green w-3 h-3 border-2 border-background"
      />
      <Handle
        type="source"
        position={Position.Right}
...
export const ActionNode = memo(({ data = {} }: any) => {
  ...
  return (
    <NodeWrapper
      ...
      icon={Icon}
      colorClass="border-border shadow-sm"
      isProcessing={data?.isProcessing}
      data={data}
    >
      <div
        className={`p-2 rounded-xl bg-background border border-border shadow-sm mb-3 w-fit ${iconColor}`}
      >
...
export const GeminiReasoningNode = memo(({ data = {} }: any) => {
  return (
    <NodeWrapper
      title={data?.label || "Gemini Reason"}
      icon={Cpu}
      colorClass="border-primary/40 shadow-lg shadow-primary/5"
      isProcessing={data?.isProcessing}
      data={data}
    >
      ...
      <Handle
        type="target"
        position={Position.Left}
        className="!bg-primary w-3.5 h-3.5 border-2 border-background ring-4 ring-primary/10"
      />
      <Handle
        type="source"
        position={Position.Right}
