...
const SortableRow = ({
  ...
  return (
    ...
    >
      <td
        className="w-8 pl-2 text-center opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
        {...attributes}
        {...listeners}
      >
      ...
      {properties.map((prop: any, propIndex: number) => {
        ...
        return (
          ...
          >
            <div
              className={`truncate max-w-[300px] text-foreground min-h-[20px] flex items-center ${isName ? "font-medium hover:underline cursor-pointer" : ""}`}
            >
              {isEditing ? (
                prop.type === "select" || prop.id === "status" ? (
                  ...
                  <input
                    ...
                    value={
                      prop.type === "date" ? formatToISO(editValue) : editValue
                    }
                    autoFocus
                    onChange={(e) => setEditValue(e.target.value)}
                    onBlur={handleBlur}
                    ...
                    className="w-full h-full bg-card outline-none px-1 rounded shadow-sm border border-primary text-sm text-foreground"
                    placeholder={
                      prop.type === "multi-select" ? "Tag1, Tag2..." : ""
                    }
                  />
                )
              ) : (
                renderCell(prop, row[prop.id], (tag) =>
                  handleRemoveTag(prop.id, tag),
                )
              )}
            </div>
          </td>
              ...
              <thead className="bg-muted/30 sticky top-0 z-10">
                <tr className="border-b border-border h-10">
                  ...
                  {properties.map((prop) => (
                    <th
                      key={prop.id}
                      className="px-3 py-2 font-normal text-muted-foreground border-r border-border last:border-r-0 hover:bg-muted/50 cursor-pointer select-none"
                    >
              ...
              <tbody className="bg-background">
                ...
                >
                  <td
                    ...
                    className="px-10 text-muted-foreground flex items-center gap-2 select-none h-full py-3"
                  >
...
const renderCell = (
  ...
  switch (property.type) {
    ...
    case "date":
      return <span className="text-xs">{formatToMMDDYYYY(value)}</span>;
    default:
      return value;
  }
};
