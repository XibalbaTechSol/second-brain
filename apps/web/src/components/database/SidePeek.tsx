          ...
          <div className="max-w-2xl mx-auto space-y-8">
                ...
                {properties.map((prop) => (
                  <React.Fragment key={prop.id}>
                    ...
                    <div className="p-1 hover:bg-muted rounded cursor-pointer min-h-[28px] flex items-center text-foreground">
                      {renderCellStatic(prop, activePage[prop.id], (tag) =>
            ...
            <div className="border-t border-border pt-8">
              <Editor
                initialContent={
                  activePage.content ||
                  `<h3>${activePage.name}</h3><p>Start writing...</p>`
                }
                onUpdate={(html) =>
                  onUpdateRow?.(activePage.id, { content: html })
                }
              />
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-1">
          {Array.isArray(value) &&
            value.map((tag: string, tagIdx: number) => (
              <span
                key={tagIdx}
                className="px-1.5 py-0.5 rounded text-[10px] bg-primary/10 text-primary border border-primary/20 flex items-center gap-1 group/tag"
              >
                <span>{tag}</span>
                {onRemoveTag && (
                  <button
