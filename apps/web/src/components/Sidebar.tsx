  ...
  return (
    <div className="w-60 bg-card dark:bg-background h-screen flex flex-col border-r border-border transition-all group/sidebar">
      ...
      <div className="p-3 mb-1">
        <Link
          href="/settings"
          className="flex items-center gap-2 px-3 py-1 rounded hover:bg-muted cursor-pointer transition-colors duration-200"
        >
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex items-center gap-2.5 px-3 py-1 rounded min-h-[30px] transition-colors duration-100 ${
                isActive
            ...
            >
              <item.icon
                className={`w-[18px] h-[18px] opacity-70 ${isActive ? "text-primary opacity-100" : ""}`}
                strokeWidth={1.5}
              />
