...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"capture" | "alerts" | "brain">(
    "capture",
  );
  ...
  const fetchData = async () => {
    try {
      ...
      if (inboxRes.ok)
        setItems((await inboxRes.ok) ? await inboxRes.json() : []);
  ...
  const nudges = items.filter(
    (i) => i.source === "SYSTEM_NUDGE" && i.status !== "COMPLETED",
  );
        ...
        >
          ...
          <View style={styles.metadataRow}>
            <Text style={styles.metadataLabel}>
              Status: {selectedEntity.task?.isDone ? "Done" : "Active"}
            </Text>
          </View>
          <Text style={styles.detailContent}>
            {selectedEntity.content || selectedEntity.summary}
          </Text>
        </ScrollView>
      </SafeAreaView>
    );
      ...
      <View style={styles.header}>
        ...
        <View style={styles.tabBar}>
          <TouchableOpacity
            ...
            style={[
              styles.tabItem,
              activeTab === "capture" && styles.tabItemActive,
            ]}
          >
            <Feather
              name="edit-3"
              size={20}
              color={activeTab === "capture" ? "#fff" : "#2563EB"}
            />
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Feather
              name="bell"
              size={20}
              color={activeTab === "alerts" ? "#fff" : "#2563EB"}
            />
            {nudges.length + needsReview.length > 0 && (
          ...
          <TouchableOpacity
            ...
            style={[
              styles.tabItem,
              activeTab === "brain" && styles.tabItemActive,
            ]}
          >
            <Feather
              name="database"
              size={20}
              color={activeTab === "brain" ? "#fff" : "#2563EB"}
            />
          </TouchableOpacity>
        </View>
      </View>
          ...
          <>
            <View style={styles.captureBox}>
              ...
              <TouchableOpacity
                style={[
                  styles.button,
                  (!inputText.trim() || loading) && styles.buttonDisabled,
                ]}
            ...
            {recent.map((item) => (
              <View key={item.id} style={styles.card}>
                ...
                <Text style={styles.cardTime}>
                  {new Date(item.createdAt).toLocaleTimeString()} • Processed
                </Text>
              </View>
            ))}
          </>
